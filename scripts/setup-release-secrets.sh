...
echo "==> Exporting secret key and uploading as MAVEN_GPG_PRIVATE_KEY"
gpg --armor --export-secret-keys "$GPG_KEY_ID" | gh secret set MAVEN_GPG_PRIVATE_KEY
...
echo "==> Uploading MAVEN_CENTRAL_USERNAME"
printf '%s' "$CENTRAL_USER" | gh secret set MAVEN_CENTRAL_USERNAME

read -rsp "Central Portal user-token PASSWORD: " CENTRAL_PASS
echo
echo "==> Uploading MAVEN_CENTRAL_PASSWORD"
printf '%s' "$CENTRAL_PASS" | gh secret set MAVEN_CENTRAL_PASSWORD
unset CENTRAL_PASS
